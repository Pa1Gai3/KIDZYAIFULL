...
const Signup: React.FC<SignupProps> = ({ setAppState, onLoginSuccess }) => {
    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-slate-50 px-4">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl border border-slate-100 p-8 md:p-12">
        ...
        <div className="text-center mb-8">
           ...
           <p className="text-slate-500 mt-2">Join KidzyColor to save your stories!</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
           ...
           >
              {isLoading ? <span className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></span> : 'Create Account'}
           </button>
        </form>
        ...
        <button 
          ...
          disabled={isLoading}
          className="w-full bg-white border border-slate-300 text-slate-700 font-bold py-3 rounded-xl flex items-center justify-center hover:bg-slate-50 transition"
        >
        ...
        <p className="text-center text-slate-600 mt-6">
          Already have an account? <button onClick={() => setAppState(AppState.LOGIN)} className="text-brand-600 font-bold hover:underline">Log In</button>
        </p>
      </div>
    </div>
  );
